...
/// How to run:
/// - dotnet run .\source\GenerationExamples.cs
/// </summary>
...
public class Program
{
    ...
    {
        Console.WriteLine($"Range(1, 5):      [{string.Join(", ", GenerationExamples.Range(1, 5))}]");
        Console.WriteLine($"Squares(5):       [{string.Join(", ", GenerationExamples.Squares(5))}]");
        Console.WriteLine($"Repeat(\"hi\", 3): [{string.Join(", ", GenerationExamples.Repeat("hi", 3))}]");
        Console.WriteLine($"Empty<int>:       [{string.Join(", ", GenerationExamples.Empty<int>())}]");
    }
}
